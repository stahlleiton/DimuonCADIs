...
void v2_fitter(int jpsiCategory      = 3, // -1 all; 1 : Prompt, 2 : Non-Prompt, 3: Bkg
	       ...
	       bool doV2ZeroFit      = false)
{
  ...
  for(int iCateg=categStart; iCateg<categEnd; iCateg++){
	...
	for(int iKin = 0; iKin < nBins; iKin++)
	  {
	    ...
	    cout<<"v2= "<<dFit_v2[iKin]<<"\t v2_err= "<<dFit_v2Err[iKin]<< "\t fit probability= " << dFitProb[iKin]<< endl;
	  }
	...
	for(int iBin = 0; iBin < nBins; iBin++)
	  {
	    ...
	    lt1->DrawLatex(0.5,0.2,Form("v_{2}= %.3f #pm %.3f",dFit_v2[iBin],dFit_v2Err[iBin]));
	    lt1->DrawLatex(0.5,0.25,Form("FitProb()= %.3f",dFitProb[iBin]));
	...
	lt1->DrawLatex(0.65,0.87,"L_{int} = 166 #mub^{-1}");
	lt1->DrawLatex(0.20,0.32,legend[iCateg]);
