...
void mergeEff() {
  ...
  for(int in = 0; in < nBin; in++){
    hDis[in] = new TH1F(Form("hDis_%d",in),"",200,0.05,0.55); // pT
    hDis[in]->Sumw2();
  ...
  sprintf(OutTextFile,"pbpb_prompt_lowpt2040.tex");
  ...
  c1->SaveAs("plot_pbpb_eff_STA_pr_dis_cent2040.png");c1->SaveAs("plot_pbpb_eff_STA_pr_dis_cent2040.pdf");
