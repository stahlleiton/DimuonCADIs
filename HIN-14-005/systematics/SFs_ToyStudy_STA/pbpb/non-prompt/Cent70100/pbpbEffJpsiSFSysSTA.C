...
TF1 *tnpSF1 = new TF1("tnpSF1","(0.9646*TMath::Erf((x-0.1260)/3.5155))/(0.9724*TMath::Erf((x-0.4114)/3.3775))",0.0,30.0);
TF1 *tnpSF2 = new TF1("tnpSF2","(0.9725*TMath::Erf((x-1.0054)/2.3187))/(0.9502*TMath::Erf((x-1.3857)/2.0757))",0.0,30.0);
TF1 *tnpSF3 = new TF1("tnpSF3","(0.9194*TMath::Erf((x-0.9733)/2.1374))/(0.8971*TMath::Erf((x-1.0984)/2.3510))",0.0,30.0);
TF1 *tnpSF4 = new TF1("tnpSF4","(0.8079*TMath::Erf((x-0.9421)/0.8577))/(0.7763*TMath::Erf((x-0.8419)/1.6742))",0.0,30.0);
...
void pbpbEffJpsiSFSysSTA__idx_(double minPt = 6.5, double maxPt = 30.0)   
{
  ...
  for(int iSpec = 0; iSpec < 1; iSpec++){
    ...
    const int nCentBins = 2; // Prompt (60-100)
    ...
    double ct_bound[nCentBins+1] = {24, 28, 40}; // Prompt (60-100)
    ...
    for(int i = 0; i < nEffBins; i++){
      ...
      dataFile<<eff[i]<<endl;//", "<<effErr[i]<<endl;
