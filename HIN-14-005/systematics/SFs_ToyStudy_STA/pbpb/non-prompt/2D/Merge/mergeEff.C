...
void mergeEff(int iCat_ = 0) {
  ...
  for(int in = 0; in < nBin; in++){
    if(iCat_==0) hDis[in] = new TH1F(Form("hDis_%d",in),"",400,0.30,0.70); // high pt 4
    if(iCat_==1) hDis[in] = new TH1F(Form("hDis_%d",in),"",400,0.30,0.70); // high pt 3
    if(iCat_==2) hDis[in] = new TH1F(Form("hDis_%d",in),"",400,0.35,0.65); // high pt 2
    if(iCat_==3) hDis[in] = new TH1F(Form("hDis_%d",in),"",400,0.45,0.65); // high pt
  ...
  if(iCat_==0) {c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y1.png");c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y1.pdf");}
  if(iCat_==1) {c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y2.png");c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y2.pdf");}
  if(iCat_==2) {c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y3.png");c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y3.pdf");}
  if(iCat_==3) {c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y4.png");c1->SaveAs("plot_pbpb_eff_STA_npr_dis_y4.pdf");}
  if(iCat_==4) {c1->SaveAs("plot_pbpb_eff_STA_npr_dis_lowpt.png");c1->SaveAs("plot_pbpb_eff_STA_npr_dis_lowpt.pdf");}
