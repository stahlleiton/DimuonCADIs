...
void mergeEff() {
  ...
  for(int in = 0; in < nBin; in++){
    ...
    hDis[in] = new TH1F(Form("hDis_%d",in),"",60,0.15,0.65); // pT
    hDis[in]->Sumw2();
  ...
  sprintf(OutTextFile,"pp_prompt_lowpt.tex");
  ...
  c1->SaveAs("plot_pp_npr_eff_STA_dis_lowpt.png");
  c1->SaveAs("plot_pp_npr_eff_STA_dis_lowpt.pdf");
