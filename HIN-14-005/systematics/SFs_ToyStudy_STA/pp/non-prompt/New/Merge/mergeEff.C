...
void mergeEff(int iCat_ = 0) {
  ...
  for(int in = 0; in < nBin; in++){
    ...
    hDis[in] = new TH1F(Form("hDis_%d",in),"",120,0.25,0.85); // pT
    hDis[in]->Sumw2();
  ...
  if(iCat_ == 0) sprintf(OutTextFile,"pp_prompt_pt.tex");
  if(iCat_ == 1) sprintf(OutTextFile,"pp_prompt_rap.tex");
  ...
  if(iCat_ == 0) {c1->SaveAs("plot_pp_npr_eff_STA_dis_pt.png");c1->SaveAs("plot_pp_npr_eff_STA_dis_pt.pdf");}
  if(iCat_ == 1) {c1->SaveAs("plot_pp_npr_eff_STA_dis_rap.png");c1->SaveAs("plot_pp_npr_eff_STA_dis_rap.pdf");}
