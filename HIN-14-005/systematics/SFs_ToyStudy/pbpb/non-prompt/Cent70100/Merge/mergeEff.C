...
void mergeEff(int iCat_ = 0) {
  ...
  for(int idir = 1; idir < 101; idir++){
    if(iCat_ == 0) in.open(Form("%s../Eff_00%d/eff_HighPt_Raps_default_Bit1_y_0.0_2.4_pT_6.5_30.0_pr.tex",dir.Data(),idir));
    Float_t x[300];
    Int_t nlines = 0;
    int nl = 0;
    while (1) {
  ...
  if(iCat_ == 0) sprintf(OutTextFile,"pbpb_non-prompt_cent70100.tex");
  ...
  if(iCat_ == 0) c1->SaveAs("plot_pbpb_npr_eff_dis_cent70100.png");c1->SaveAs("plot_pbpb_npr_eff_dis_cent70100.pdf");
