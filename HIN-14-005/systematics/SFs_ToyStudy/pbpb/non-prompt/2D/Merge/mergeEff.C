...
void mergeEff(int iCat_ = 0) {
  ...
  for(int in = 0; in < nBin; in++){
    if(iCat_==0) hDis[in] = new TH1F(Form("hDis_%d",in),"",120,0.30,0.70); // high pt 4
    if(iCat_==1) hDis[in] = new TH1F(Form("hDis_%d",in),"",120,0.30,0.70); // high pt 3
    if(iCat_==2) hDis[in] = new TH1F(Form("hDis_%d",in),"",90,0.35,0.65); // high pt 2
    if(iCat_==3) hDis[in] = new TH1F(Form("hDis_%d",in),"",60,0.45,0.65); // high pt
    hDis[in] = new TH1F(Form("hDis_%d",in),"",140,0.00,0.70); // low pt
    hDis[in]->Sumw2();
  ...
  if(iCat_ == 0) sprintf(OutTextFile,"pbpb_non-prompt_y_0.0_1.2_pT_6.5_30.tex");
  if(iCat_ == 1) sprintf(OutTextFile,"pbpb_non-prompt_y_1.2_1.6_pT_6.5_30.tex");
  if(iCat_ == 2) sprintf(OutTextFile,"pbpb_non-prompt_y_1.6_2.4_pT_6.5_30.tex");
  if(iCat_ == 3) sprintf(OutTextFile,"pbpb_non-prompt_y_1.6_2.4_pT_3.0_6.5.tex");
  if(iCat_ == 4) sprintf(OutTextFile,"pbpb_non-prompt_y_0.0_2.4_pT_6.5_30.tex");
  ...
  if(iCat_==0) {c1->SaveAs("plot_pbpb_npr_eff_dis_y1.png");c1->SaveAs("plot_pbpb_npr_eff_dis_y1.pdf");}
  if(iCat_==1) {c1->SaveAs("plot_pbpb_npr_eff_dis_y2.png");c1->SaveAs("plot_pbpb_npr_eff_dis_y2.pdf");}
  if(iCat_==2) {c1->SaveAs("plot_pbpb_npr_eff_dis_y3.png");c1->SaveAs("plot_pbpb_npr_eff_dis_y3.pdf");}
  if(iCat_==3) {c1->SaveAs("plot_pbpb_npr_eff_dis_y4.png");c1->SaveAs("plot_pbpb_npr_eff_dis_y4.pdf");}
  if(iCat_==4) {c1->SaveAs("plot_pbpb_npr_eff_dis_lowpt.png");c1->SaveAs("plot_pbpb_npr_eff_dis_lowpt.pdf");}
