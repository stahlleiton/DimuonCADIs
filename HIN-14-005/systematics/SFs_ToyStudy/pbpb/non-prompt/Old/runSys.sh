...
cat ../fnc_pbpb_eta1.txt >& tlist1
cat ../fnc_pbpb_eta2.txt >& tlist2
cat ../fnc_pbpb_eta3.txt >& tlist3
cat ../fnc_pbpb_eta4.txt >& tlist4
...
do
  echo $i
  awk -v p=$line1 '{gsub("_fnc1_",p); print}' pbpbEffJpsi.C > del_eff_$i.C;
  i=$(expr $i + 1);
...
do
  ...
  awk -v p=$line4 '{gsub("_fnc4_",p); print}' del_eff4_$i.C > Eff_00$i/pbpbEffJpsi_$i.C
