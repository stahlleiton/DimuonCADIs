...
void mergeEff() {
  ...
  for(int in = 0; in < nBin; in++){
    ...
    hDis[in] = new TH1F(Form("hDis_%d",in),"",150,0.00,0.5); // pT
    hDis[in]->Sumw2();
  ...
  sprintf(OutTextFile,"pbpb_non-prompt_lowpt.tex");
  ...
  c1->SaveAs("plot_pbpb_npr_eff_dis_lowpt.png");c1->SaveAs("plot_pbpb_npr_eff_dis_lowpt.pdf");
