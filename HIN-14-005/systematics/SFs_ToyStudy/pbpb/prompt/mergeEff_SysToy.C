// This is the macro to draw the summary plots for T&P Scale Factor systematics
...
#include "Riostream.h"
void mergeEff_SysToy(int iCat_ = 0) {
  TString dir = gSystem->UnixPathName(gInterpreter->GetCurrentMacroName());
  dir.ReplaceAll("mergeEff.C","");
  dir.ReplaceAll("/./","/");
  ifstream in;
  const int nBin = 7; // Cent(13), pT(7), y(6)
  ...
  for(int in = 0; in < nBin; in++){
    ...
    hDis[in] = new TH1F(Form("hDis_%d",in),"",200,0.25,0.85); // pT
    hDis[in]->Sumw2();
  ...
  if(iCat_ == 0) sprintf(OutTextFile,"pp_non-prompt_pt.tex");
  if(iCat_ == 1) sprintf(OutTextFile,"pp_non-prompt_rap.tex");
  ...
  for(int ic = 1; ic < nBin+1; ic++){
    c1->cd(ic);
    hDis[ic-1]->GetXaxis()->SetRangeUser(0.2,0.8);
    hDis[ic-1]->GetXaxis()->SetTitle("Efficiency");
    //if(ic==1) hDis[ic-1]->GetXaxis()->SetRangeUser(0.0,0.4);
    //if(ic==2) hDis[ic-1]->GetXaxis()->SetRangeUser(0.0,0.4);
    hDis[ic-1]->Draw("E");
    ...
    if(iCat_==1) lt->DrawLatex(0.2,0.95,Form("pp, p_{T} [6.5,30.0], y [%0.1f,%0.1f]",rap_bound[ic-1],rap_bound[ic]));
  }

  if(iCat_ == 0) {c1->SaveAs("plot_pp_npr_eff_dis_pt.png");c1->SaveAs("plot_pp_npr_eff_dis_pt.pdf");}
  if(iCat_ == 1) {c1->SaveAs("plot_pp_npr_eff_dis_rap.png");c1->SaveAs("plot_pp_npr_eff_dis_rap.pdf");}
