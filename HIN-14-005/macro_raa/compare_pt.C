              ...
              const char* outputDir = "figs/compare")// where the output figures will be
{
 ...
 // same for the raa denominator and nominator yields histograms
  ...
  if(bSavePlots)
  {
    ...
    c1->SaveAs(Form("%s/PrJpsi_vsPt_%s.root",outputDir,compWhat[whichCompare]));
  }
  ...
  if(bSavePlots)
  {
    ...
    c11b->SaveAs(Form("%s/PrJpsi_vsPt_mb_%s.root",outputDir,compWhat[whichCompare]));
  }
  ...
  if(bSavePlots)
  {
    ...
    c2->SaveAs(Form("%s/nonPrJpsi_vsPt_%s.root",outputDir,compWhat[whichCompare]));
  }
  ...
  if(bSavePlots)
  {
    ...
    c22b->SaveAs(Form("%s/nonPrJpsi_vsPt_mb_%s.root",outputDir,compWhat[whichCompare]));
  }
