...
Input:
 *.dat and *.root produced by v2_fitter.C macro
Output:
 *.dat and *.root files, containing the total uncert, as well as individual contributions
...
void makeSyst_pt(
		 ...
		 bool doMb             = true, // if calculate the minbias point (for pnNumBinsHighPt, this is done already)
		 string nDphiBins      = "4",
  ...
  int nBins = nPtBins_pr; 
  if(jpsiCategory==2) nBins   = nPtBins_np; // low-pt + high-pt
  if(doMb) nBins   = 1; // 1 bin if MB
  ...
  for(int iFile = 0; iFile<nFiles; iFile++)
    {
      ifstream in;
      std::string nameVar   = outFilePlot[1]; //pt
      std::string nameSig   = signal[jpsiCategory];// pr, npr, bkg
	...
	cout << "!!!!!! Input file name for minbias: "<< nameDir <<"/\t"<< inputFile_mb <<endl;
  ...
  std::string nameVar   = outFilePlot[1]; //pt
  if(doMb) nameVar      = outFilePlot[0]; //mb
