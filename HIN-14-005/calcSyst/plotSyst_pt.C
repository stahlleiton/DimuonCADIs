 ...
 // leg22a->AddEntry(gNonPrJpsiSyst_mb,"|y|<2.4","f");
  ...
  //------------------------------------------------------------
      ...
      for(int ivar=0; ivar<(nFitVariations+nEff4DVariations); ivar++)
	{
	  ...
	  // high-pt bins
	  TGraph *gPrJpsiSyst_aa  = new TGraph(numBins, binsPt, relVar_pr_aa);
	  TGraph *gNPrJpsiSyst_aa = new TGraph(numBins, binsPt, relVar_npr_aa);
	  TGraph *gPrJpsiSyst_pp  = new TGraph(numBins, binsPt, relVar_pr_pp);
	  TGraph *gNPrJpsiSyst_pp = new TGraph(numBins, binsPt, relVar_npr_pp);
