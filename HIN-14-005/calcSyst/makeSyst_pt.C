                  ...
                  const char* outputDir = "histSyst")// where the output figures will be
{
  ...
  string ptbins_lowpt_str[] = {"3045","4555","5565"};
  ...
  { 
    ...
    //  for(int ibin=1; ibin<=2; ibin++)
    {
      ...
      const double scale_cent     = 1/(adTaaMB[0]*adDeltaCentMB[0]);
      const double yieldRatio_pr  = yield_aa_pr/yield_pp_pr   * scaleFactor * scale_cent;
      const double yieldRatio_npr = yield_aa_npr/yield_pp_npr * scaleFactor * scale_cent;
      ...
      for(int ivar=0; ivar<(nFitVariations+nEff4DVariations+nEffTnPVariation); ivar++)
      {
        ...
        case 0: // high-pt
          ...
          if(ivar<nFitVariations)
          {
            ...
            if(method==0) // rms of all variations for the fit methods all the time
            {
          ...
          if(ivar>=nFitVariations && ivar<(nFitVariations+nEff4DVariations) )
          {
            ...
            if(method==0) // add in quadrature, this is independent
            {
        ...
        case 2:// fwd, low-pt, high-pt, low+high
          ...
          if(ivar<nFitVariations)
          {
            ...
            if(bDoDebug) cout << "++++++++++++++++++++++ Delta yields: "
                              << syst_fit_y1624MB_pr_aa[ibin-1][ivar] << "\t& " << syst_fit_y1624MB_npr_aa[ibin-1][ivar] << "\t; "
                              << syst_fit_y1624MB_pr_pp[ibin-1][ivar] << "\t& " << syst_fit_y1624MB_npr_pp[ibin-1][ivar] << endl;
            ...
            if(bDoDebug) cout << "+++++++++++++++++++++++++++++++++ Fit contribution to systm: "
                              << fitContribution_y1624MB_pr_aa << "\t& " << fitContribution_y1624MB_npr_aa << "\t; "
                              << fitContribution_y1624MB_pr_pp << "\t& " << fitContribution_y1624MB_npr_pp << endl;
          }
          if(ivar>=nFitVariations && ivar<(nFitVariations+nEff4DVariations) )
          {
            int ifile = ivar-nFitVariations;
            ...
            if(bDoDebug) cout << "++++++++++++++++++++++ Delta yields: "
                              << syst_eff4d_y1624MB_pr_aa[ibin-1][ifile] << "\t& " << syst_eff4d_y1624MB_npr_aa[ibin-1][ifile] << "\t; "
                              << syst_eff4d_y1624MB_pr_pp[ibin-1][ifile] << "\t& " << syst_eff4d_y1624MB_npr_pp[ibin-1][ifile] << endl;
            ...
            if(bDoDebug) cout << "+++++++++++++++++++++++++++++++++ Fit contribution to systm: "
                              << eff4dContribution_y1624MB_pr_aa << "\t& " << eff4dContribution_y1624MB_npr_aa << "\t; "
                              << eff4dContribution_y1624MB_pr_pp << "\t& " << eff4dContribution_y1624MB_npr_pp << endl;
          }
          if( (ivar>=(nFitVariations+nEff4DVariations)) && (ivar < (nFitVariations+nEff4DVariations+nEffTnPVariation)) )
          {
            int ifile = ivar-nFitVariations-nEff4DVariations;
            ...
            if(bDoDebug) cout << "++++++++++++++++++++++ Delta yields: "
                              << syst_effTnP_y1624MB_pr_aa[ibin-1][ifile] << "\t& " << syst_effTnP_y1624MB_npr_aa[ibin-1][ifile] << "\t; "
                              << syst_effTnP_y1624MB_pr_pp[ibin-1][ifile] << "\t& " << syst_effTnP_y1624MB_npr_pp[ibin-1][ifile] << endl;
            
            if(method==1)//maximum
            {
              if( syst_effTnP_y1624MB_pr_aa[ibin-1][ifile] > efftnpContribution_y1624MB_pr_aa ) efftnpContribution_y1624MB_pr_aa = syst_effTnP_y1624MB_pr_aa[ibin-1][ifile];
              if( syst_effTnP_y1624MB_pr_pp[ibin-1][ifile] > efftnpContribution_y1624MB_pr_pp ) efftnpContribution_y1624MB_pr_pp = syst_effTnP_y1624MB_pr_pp[ibin-1][ifile];
            ...
            if(bDoDebug) cout << "+++++++++++++++++++++++++++++++++ Fit contribution to systm: "
                              << efftnpContribution_y1624MB_pr_aa << "\t& " << efftnpContribution_y1624MB_npr_aa << "\t; "
                              << efftnpContribution_y1624MB_pr_pp << "\t& " << efftnpContribution_y1624MB_npr_pp << endl;
          }
          break;
      ...
      case 0:
        ...
        outputData_pr << ptbins_str[ibin-1]<<"\t" << "0024\t" << "0100\t" << yieldRatio_pr << "\t" << prJpsiErrSyst_pt[ibin-1] << "\t"
      ...
      case 1:
        ...
        nonPrJpsiErrSyst_pt365y1624_pt[ibin-1] = yieldRatio_npr * TMath::Sqrt((fitContribution_pt365y1624_npr_aa/rms_fitContribNorm+eff4dContribution_pt365y1624_npr_aa+efftnpContribution_pt365y1624_npr_aa)+
                                                                              (fitContribution_pt365y1624_npr_pp/rms_fitContribNorm+eff4dContribution_pt365y1624_npr_pp+efftnpContribution_pt365y1624_npr_pp));  

        outputData_pr << ptbins_lowpt_str[ibin-1]<<"\t" << "1624\t" << "0100\t" << yieldRatio_pr << "\t" << prJpsiErrSyst_pt365y1624_pt[ibin-1] << "\t"
      ...
      case 2: 
        prJpsiErrSyst_y1624MB_pt[ibin-1] = yieldRatio_pr * TMath::Sqrt((fitContribution_y1624MB_pr_aa/rms_fitContribNorm+eff4dContribution_y1624MB_pr_aa+efftnpContribution_y1624MB_pr_aa)+
                                                                       (fitContribution_y1624MB_pr_pp/rms_fitContribNorm+eff4dContribution_y1624MB_pr_pp+efftnpContribution_y1624MB_pr_pp));
        nonPrJpsiErrSyst_y1624MB_pt[ibin-1] = yieldRatio_npr * TMath::Sqrt((fitContribution_y1624MB_npr_aa/rms_fitContribNorm+eff4dContribution_y1624MB_npr_aa+efftnpContribution_y1624MB_npr_aa)+
                                                                           (fitContribution_y1624MB_npr_pp/rms_fitContribNorm+eff4dContribution_y1624MB_npr_pp+efftnpContribution_y1624MB_npr_pp));  
      ...
      case 3:
        prJpsiErrSyst_mb[0] = yieldRatio_pr * TMath::Sqrt((fitContribution_mb_pr_aa/rms_fitContribNorm+eff4dContribution_mb_pr_aa+efftnpContribution_mb_pr_aa)+
                                                          (fitContribution_mb_pr_pp/rms_fitContribNorm+eff4dContribution_mb_pr_pp+efftnpContribution_mb_pr_pp));
        nonPrJpsiErrSyst_mb[0] = yieldRatio_npr *  TMath::Sqrt((fitContribution_mb_npr_aa/rms_fitContribNorm+eff4dContribution_mb_npr_aa+efftnpContribution_mb_npr_aa)+
