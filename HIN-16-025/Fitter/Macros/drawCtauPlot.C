...
void setCtauRange        ( RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale, const vector<double>& rangeErr, const double excEvts=0.0 );
void printCtauParameters ( TPad& pad, const RooWorkspace& myws, const bool& isPbPb, const string& pdfName, const bool& isWeighted );
...
void drawCtauPlot(RooWorkspace& myws,          // Local workspace
		  ...
		  const struct InputOpt& opt,  // Variable with run information (kept for legacy purpose)
                  ...
                  const bool& incNonPrompt,    // Includes Non-Prompt ctau model
                  // Select the fitting options
                  const bool& useSPlot,        // If yes, then use SPlot ds, if no, use mass range one
                  ...
                  ) 
{
  ...
  const bool usectauBkgTemplate = incBkg && !(myws.pdf(Form("pdfCTAU_BkgPR_%s", (isPbPb?"PbPb":"PP")))) && !(myws.pdf(Form("pdfCTAU_BkgNoPR_%s", (isPbPb?"PbPb":"PP"))));
  ...
  const string pdfTotName  = Form("pdfCTAU_Tot_%s", (isPbPb?"PbPb":"PP"));
  string dsOSName = Form("dOS_%s_%s", DSTAG.c_str(), (isPbPb?"PbPb":"PP"));
  if (plotPureSMC) dsOSName = Form("dOS_%s_%s_NoBkg", DSTAG.c_str(), (isPbPb?"PbPb":"PP"));
  const string dsOSNameCut = dsOSName+"_CTAUCUT";
  ...
  const string dsOSName2FitCut = dsOSName2Fit+"_CTAUCUT";
  const string hOSName = Form("dhCTAUERRTot_Tot_%s", (isPbPb?"PbPb":"PP"));
  const string hOSNameBkg  = Form("dhCTAUERR_Bkg_%s", (isPbPb?"PbPb":"PP"));
  const string hOSNameJpsi = Form("dhCTAUERR_Jpsi_%s", (isPbPb?"PbPb":"PP"));
  const string hOSNamePsi2S = Form("dhCTAUERR_Psi2S_%s", (isPbPb?"PbPb":"PP"));
  ...
  const double numTot = myws.data(dsOSName2Fit.c_str())->sumEntries();
  const double outTot = myws.data(dsOSName2Fit.c_str())->numEntries();
  auto dsRed = std::unique_ptr<RooAbsData>(myws.data(dsOSName2Fit.c_str())->reduce(Form("(ctau>%.6f || ctau<%.6f)", range[1], range[0])));
  ...
  auto frame = std::unique_ptr<RooPlot>(myws.var("ctau")->frame(Bins(nBins), Range(minRange, maxRange)));  
  frame->updateNormVars(RooArgSet(*myws.var("invMass"), *myws.var("ctau"), *myws.var("ctauErr"))) ;
  myws.data(dsOSName2Fit.c_str())->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
  
  if (incBkg && (!incJpsi && !incPsi2S)) {
    if (!usectauBkgTemplate)
    {
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKG"),  Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(32),
      ...
      myws.data(dsOSName2Fit.c_str())->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
      if (myws.pdf(Form("pdfCTAU_BkgPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKGPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_BkgPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                          ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE),
                                                                                                          Normalization(normBkg, RooAbsReal::NumEvent),
                                                                                                          LineColor(kRed+2), Precision(1e-4), NumCPU(32)
                                                                                                          );
      }
      if (myws.pdf(Form("pdfCTAU_BkgNoPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKGNoPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_BkgNoPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                            ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE),
                                                                                                            Normalization(normBkg, RooAbsReal::NumEvent),
                                                                                                            LineColor(kOrange+10), Precision(1e-4), NumCPU(32)
                                                                                                            );
      }
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(32),
                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameBkg.c_str()), kTRUE),
    ...
    {
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(numTot, RooAbsReal::NumEvent), NumCPU(32),
                                           LineColor(kBlack), Precision(1e-4)
                                           );
    }
  }
  if (useSPlot) {
    if (incJpsi) {
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("JPSI"),  Normalization(normJpsi, RooAbsReal::NumEvent), NumCPU(32),
                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE),
                                           FillStyle(1001), FillColor(kAzure-9), VLines(), DrawOption("LCF"), Precision(1e-4)
                                           );
      myws.data(dsOSName2Fit.c_str())->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
      if (myws.pdf(Form("pdfCTAU_JpsiPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("JPSIPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_JpsiPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE),
                                                                                                           Normalization(normJpsi, RooAbsReal::NumEvent),
                                                                                                           LineColor(kRed+2), Precision(1e-4), NumCPU(32)
                                                                                                           );
      }
      if (myws.pdf(Form("pdfCTAU_JpsiNoPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKGNoPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_JpsiNoPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                             ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE),
                                                                                                             Normalization(normJpsi, RooAbsReal::NumEvent),
                                                                                                             LineColor(kOrange+10), Precision(1e-4), NumCPU(32)
                                                                                                             );
      }
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(normJpsi, RooAbsReal::NumEvent), NumCPU(32),
                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNameJpsi.c_str()), kTRUE),
                                           LineColor(kBlack), Precision(1e-4)
                                           );
    }
    if (incPsi2S) {
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("PSI2S"),  Normalization(normPsi2S, RooAbsReal::NumEvent), NumCPU(32),
                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE),
                                           FillStyle(1001), FillColor(kAzure-9), VLines(), DrawOption("LCF"), Precision(1e-4)
                                           );
      myws.data(dsOSName2Fit.c_str())->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
      if (myws.pdf(Form("pdfCTAU_Psi2SPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("JPSIPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_Psi2SPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE),
                                                                                                           Normalization(normPsi2S, RooAbsReal::NumEvent),
                                                                                                           LineColor(kRed+2), Precision(1e-4), NumCPU(32)
                                                                                                           );
      }
      if (myws.pdf(Form("pdfCTAU_Psi2SNoPR_%s", (isPbPb?"PbPb":"PP")))) {myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKGNoPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAU_Psi2SNoPR_%s", (isPbPb?"PbPb":"PP"))))),
                                                                                                             ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE),
                                                                                                             Normalization(normPsi2S, RooAbsReal::NumEvent),
                                                                                                             LineColor(kOrange+10), Precision(1e-4), NumCPU(32)
                                                                                                             );
      }
      myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(normPsi2S, RooAbsReal::NumEvent), NumCPU(32),
                                           ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(hOSNamePsi2S.c_str()), kTRUE),
                                           LineColor(kBlack), Precision(1e-4)
                                           );
    }
  ...
  hpull->SetName("hpull");
  auto frame2 = std::unique_ptr<RooPlot>(myws.var("ctau")->frame(Title("Pull Distribution"), Bins(nBins), Range(minRange, maxRange)));
  frame2->addPlotable(hpull, "PX");
  
  // Create the main canvas
  auto cFig  = std::unique_ptr<TCanvas>(new TCanvas(Form("cCtauFig_%s", (isPbPb?"PbPb":"PP")), "cCtauFig",800,800));
  cFig->cd();
  ...
  setCtauRange(*frame, myws, dsOSName2FitCut, setLogScale, range, outErr);
  ...
  if (!usectauBkgTemplate) printCtauParameters(*pad1, myws, isPbPb, pdfTotName, isWeighted);
...
void setCtauRange(RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale, const vector<double>& rangeErr, const double excEvts)
{ 
  // Find maximum and minimum points of Plot to rescale Y axis
  auto h = std::unique_ptr<TH1>(myws.data(dsName.c_str())->createHistogram("hist", *myws.var("ctau"), Binning(frame.GetNbinsX(),frame.GetXaxis()->GetXmin(),frame.GetXaxis()->GetXmax())));
