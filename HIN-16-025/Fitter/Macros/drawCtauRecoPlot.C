...
void setCtauRecoRange ( RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale );
                      ...
                      ) 
{
  ...
  const string pdfName  = "pdfCTAU";
  const string pdfTotName  = Form("%s_%s_%s", pdfName.c_str(), incJpsi?"JpsiNoPR":"Psi2SNoPR", (isPbPb?"PbPb":"PP"));
  ...
  auto frame = std::unique_ptr<RooPlot>(myws.var("ctau")->frame(Bins(nBins), Range(cut.dMuon.ctauTrue.Min, cut.dMuon.ctauTrue.Max)));
  ...
  hpull->SetName("hpull");
  auto frame2 = std::unique_ptr<RooPlot>(myws.var("ctau")->frame(Title("Pull Distribution"), Bins(nBins), Range(cut.dMuon.ctauTrue.Min, cut.dMuon.ctauTrue.Max)));
...
;
  ...
  auto cFig  = std::unique_ptr<TCanvas>(new TCanvas(Form("cCtauRecoFig_%s", (isPbPb?"PbPb":"PP")), "cCtauRecoFig",800,800));
  cFig->cd();
  ...
  setCtauRecoRange(*frame, myws, dsOSName, setLogScale);
  ...
  // *** Print chi2/ndof 
  printChi2(myws, *pad2, *frame, "ctau", dsOSName, pdfTotName, nBins);
