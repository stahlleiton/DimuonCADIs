...
void setMassFrom2DRange        ( RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale );
void printMassFrom2DParameters ( TPad& Pad, const RooWorkspace& myws, const bool& isPbPb, const string& pdfName, const bool& isWeighted );
                        ...
                        )
{
  ...
  const string pdfJpsiPRName  = Form("pdfCTAUMASS_JpsiPR_%s", (isPbPb?"PbPb":"PP"));
  const string pdfJpsiNoPRName  = Form("pdfCTAUMASS_JpsiNoPR_%s", (isPbPb?"PbPb":"PP"));
  const string pdfPsi2SPRName  = Form("pdfCTAUMASS_Psi2SPR_%s", (isPbPb?"PbPb":"PP"));
  const string pdfPsi2SNoPRName  = Form("pdfCTAUMASS_Psi2SNoPR_%s", (isPbPb?"PbPb":"PP"));
  ...
  myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("BKG"),Components(RooArgSet(*myws.pdf(Form("pdfMASS_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
  ...
  if (!paperStyle) {
    if (incJpsi) {
      if ( myws.pdf(Form("pdfCTAUMASS_JpsiPR_%s", (isPbPb?"PbPb":"PP"))) ) {
        myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("JPSIPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAUMASS_JpsiPR_%s", (isPbPb?"PbPb":"PP"))), *myws.pdf(Form("pdfCTAUMASS_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
      ...
      if ( myws.pdf(Form("pdfCTAUMASS_JpsiNoPR_%s", (isPbPb?"PbPb":"PP"))) ) {
        myws.pdf(pdfTotName.c_str())->plotOn(frame.get(),Name("JPSINOPR"),Components(RooArgSet(*myws.pdf(Form("pdfCTAUMASS_JpsiNoPR_%s", (isPbPb?"PbPb":"PP"))), *myws.pdf(Form("pdfCTAUMASS_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                                             ProjWData(RooArgSet(*myws.var("ctauErr")), *myws.data(dsOSName.c_str()), kTRUE),
                                             Normalization(normDSTot, RooAbsReal::NumEvent),
  ...
  setMassFrom2DRange(*frame, myws, dsOSName, setLogScale);
  if (paperStyle) {
  ...
  printMassFrom2DParameters(*pad1, myws, isPbPb, pdfTotName, isWeighted);
  ...
  if (!paperStyle) {
     ...
     frame2->Draw();
     // *** Print chi2/ndof 
     printChi2(myws, *pad2, *frameTMP, "invMass", dsOSName, pdfTotName, nBinsTMP, false);
     pline->Draw("same");
     pad2->Update();
  }
