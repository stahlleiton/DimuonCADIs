...
void setMassRange        ( RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale,
                           const double dMuonYmin=-1., const double dMuonYmax=-1., const bool plotPureSMC=false, const bool cutSideBand=false );
void printMassParameters ( TPad& pad, const RooWorkspace& myws, const bool& isPbPb, const string& pdfName, const bool& isWeighted );
...
void drawMassPlot(RooWorkspace& myws,   // Local workspace
                  ...
                  const bool  getMeanPT=false, // Compute the mean PT (NEED TO FIX)
                  const bool  paperStyle=false // if true, print less info
                  ) 
{
  ...
  if(applyWeight_Corr)
  {
    ...
    dsOSName = Form("dOS_%s_%s_%s", DSTAG.c_str(),(isPbPb?"PbPb":"PP"), corrName.c_str());
  }

  const string pdfName = Form("pdfMASS_Tot_%s", (isPbPb?"PbPb":"PP"));
  ...
  const string cutSB = (parIni.count("BkgMassRange_FULL_Cut")>0 ? parIni.at("BkgMassRange_FULL_Cut") : "");
  const string cutSBLabel = (parIni.count("BkgMassRange_FULL_Label")>0 ? parIni.at("BkgMassRange_FULL_Label") : "");
 
  // Create the main plot of the fit
  auto frame = std::unique_ptr<RooPlot>(myws.var("invMass")->frame(Bins(nBins), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)));
  ...
  double norm = myws.data(dsOSName.c_str())->sumEntries();
  if (plotPureSMC)
  {
    if (incJpsi && !incPsi2S)
    {
      if ( (cut.dMuon.AbsRap.Min >= 1.6) || (cut.dMuon.AbsRap.Max > 1.6) ) { auto ds = std::unique_ptr<RooAbsData>(myws.data(dsOSName.c_str())->reduce("invMass<3.32")); norm = ds->sumEntries(); }
      else { auto ds = std::unique_ptr<RooAbsData>(myws.data(dsOSName.c_str())->reduce("invMass<3.26")); norm = ds->sumEntries(); }
    }
    else if (!incJpsi && incPsi2S)
    {
      if (cut.dMuon.AbsRap.Min >= 1.6) { auto ds = std::unique_ptr<RooAbsData>(myws.data(dsOSName.c_str())->reduce("invMass<3.95")); norm = ds->sumEntries(); }
      else { auto ds = std::unique_ptr<RooAbsData>(myws.data(dsOSName.c_str())->reduce("invMass<3.85")); norm = ds->sumEntries(); }
    }
  }
     ...
     if (incJpsi) {
        if (incBkg) {
           if ( myws.pdf(Form("pdfMASS2_Jpsi_%s", (isPbPb?"PbPb":"PP"))) ) {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI1"), Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Jpsi_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
              ...
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Jpsi_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kOrange+2), LineStyle(1), Precision(1e-4)
                    );
           } else {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Jpsi_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
           }
           ...
           if ( myws.pdf(Form("pdfMASS2_Jpsi_%s", (isPbPb?"PbPb":"PP"))) ) {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI1"),Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Jpsi_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Jpsi_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kOrange+2), LineStyle(1), Precision(1e-4)
                    );
           } else {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("JPSI1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Jpsi_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
           }
     ...
     if (incPsi2S) {
        if (incBkg) {
           if ( myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))) ) {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                    LineColor(kOrange+2), LineStyle(1), Precision(1e-4)
                    );
           } else {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
           }
           ...
           if ( myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))) ) {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kOrange+2), LineStyle(1), Precision(1e-4)
                    );
           } else {
              myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                    Normalization(norm, RooAbsReal::NumEvent), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange("MassWindow"),
                    LineColor(kGreen+3), LineStyle(1), Precision(1e-4)
                    );
           }
  ...
  if (incBkg && (!incJpsi && !incPsi2S)) {
    if (!paperStyle) {
       ((RooDataSet*)myws.data(dsOSName.c_str())->reduce(cutSB.c_str()))->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
       myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("BKG"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
  ...
  if (incBkg && (!incJpsi && !incPsi2S)) {
    if (!paperStyle) {
       ((RooDataSet*)myws.data(dsOSName.c_str())->reduce(cutSB.c_str()))->plotOn(frame.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
       myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(myws.data(dsOSName.c_str())->reduce(cutSB.c_str())->sumEntries(), RooAbsReal::NumEvent), 
             LineColor(kBlack), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange(cutSBLabel.c_str()));
    } else {
       myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(myws.data(dsOSName.c_str())->sumEntries(), RooAbsReal::NumEvent), 
                                      LineColor(kBlack), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max), NormRange(cutSBLabel.c_str()));
    }
  } else {
    myws.pdf(pdfName.c_str())->plotOn(frame.get(),Name("PDF"),  Normalization(norm, RooAbsReal::NumEvent), 
  ...
  hpull->SetName("hpull");
  auto frame2 = std::unique_ptr<RooPlot>(myws.var("invMass")->frame(Title("Pull Distribution"), Bins(nBins), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)));
  frame2->addPlotable(hpull, "PX"); 

  // Create the extra PAD for the Psi(2S) zoom
  std::unique_ptr<RooPlot> framezoom;
  if(zoomPsi) {  
    setLogScale=false;
    framezoom.reset(myws.var("invMass")->frame(Bins(22), Range(Mass.Psi2S-0.275, Mass.Psi2S+0.275)));
    myws.data(dsOSName.c_str())->plotOn(framezoom.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2)); 
    if (!paperStyle) {
       if (incPsi2S) {
          if (incBkg) {
             if ( myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))) ) {
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kGreen+3), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kOrange+2), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
             } else {
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Psi2S_%s", (isPbPb?"PbPb":"PP"))),*myws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kGreen+3), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
             }
             ...
             if ( myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))) ) {
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASS1_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kGreen+3), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S2"),Components(RooArgSet(*myws.pdf(Form("pdfMASS2_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kOrange+2), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
             } else {
                myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PSI2S1"),Components(RooArgSet(*myws.pdf(Form("pdfMASSTot_Psi2S_%s", (isPbPb?"PbPb":"PP"))))),
                      Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                      LineColor(kGreen+3), LineStyle(1), Precision(1e-4), Range(cut.dMuon.M.Min, cut.dMuon.M.Max)
                      );
             }
    ...
    if (incBkg) {
      myws.pdf(pdfName.c_str())->plotOn(framezoom.get(), Name("BKG"),Components(*myws.pdf(Form("pdfMASS_Bkg_%s", (isPbPb?"PbPb":"PP")))),
                                        Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"), 
                                        FillStyle(paperStyle ? 0 : 1001), FillColor(kAzure-9), VLines(), DrawOption("LCF"), LineColor(kBlue), LineStyle(kDashed));
    }
    myws.data(dsOSName.c_str())->plotOn(framezoom.get(), Name("dOS"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(1.2));
    myws.pdf(pdfName.c_str())->plotOn(framezoom.get(),Name("PDF"), Normalization(norm, RooAbsReal::NumEvent), NormRange("MassWindow"),
  ...
  auto cFig  = std::unique_ptr<TCanvas>(new TCanvas(Form("cMassFig_%s", (isPbPb?"PbPb":"PP")), "cMassFig",800,800));
  cFig->cd();
  auto pad1  = new TPad(Form("pad1_%s", (isPbPb?"PbPb":"PP")),"",0,paperStyle ? 0 : 0.23,1,1);
  auto pad2  = new TPad(Form("pad2_%s", (isPbPb?"PbPb":"PP")),"",0,0,1,.228);
  auto pline = std::unique_ptr<TLine>(new TLine(cut.dMuon.M.Min, 0.0, cut.dMuon.M.Max, 0.0));
  
  // TPad *pad4 = new TPad("pad4","This is pad4",0.55,0.46,0.97,0.87);
  auto pad4 = new TPad("pad4","This is pad4",0.55,paperStyle ? 0.29 : 0.36,0.97,paperStyle ? 0.70 : 0.77);
  pad4->SetFillStyle(0);
  ...
  setMassRange(*frame, myws, dsOSName, setLogScale, cut.dMuon.AbsRap.Min, cut.dMuon.AbsRap.Max, plotPureSMC, SB);
  if (paperStyle) {
     const double Ydown = 0.;//frame->GetMinimum();
     const double Yup = 0.9*frame->GetMaximum();
  ...
  if (!zoomPsi) { printMassParameters(*pad1, myws, isPbPb, pdfName, isWeighted); }
  ...
  if (zoomPsi) {
    ...
    setMassRange(*framezoom, myws, dsOSName, setLogScale, cut.dMuon.AbsRap.Min, cut.dMuon.AbsRap.Max, plotPureSMC);
    pad4->Draw();
    pad4->cd();
    framezoom->Draw();    
    cFig->cd(); 
  ...
  if (!paperStyle) {
     ...
     frame2->Draw(); 
     // *** Print chi2/ndof 
     printChi2(myws, *pad2, *frameTMP, "invMass", dsOSName, pdfName, nBins);
     pline->Draw("same");
     pad2->Update();
  }
...
void setMassRange(RooPlot& frame, const RooWorkspace& myws, const string& dsName, const bool& setLogScale,
                  const double dMuonYmin, const double dMuonYmax, const bool plotPureSMC, const bool cutSideBand)
{ 
  // Find maximum and minimum points of Plot to rescale Y axis
  auto h = std::unique_ptr<TH1>(myws.data(dsName.c_str())->createHistogram("hist", *myws.var("invMass"), Binning(frame.GetNbinsX(),frame.GetXaxis()->GetXmin(),frame.GetXaxis()->GetXmax())));
...
void printMassParameters(TPad& pad, const RooWorkspace& myws, const bool& isPbPb, const string& pdfName, const bool& isWeighted)
{
  ...
  auto par = std::unique_ptr<RooArgSet>(myws.pdf(pdfName.c_str())->getParameters(*myws.var("invMass")));
