...
void gen1(const char* output, const char* suffix) {
  ...
  ws->factory(Form("Gaussian::sig_nonorm_%s(invMass,m0_%s[3,2.5,3.5],sigma_%s[0.1,0.,0.5])",suffix,suffix,suffix));
  ...
  RooAbsPdf *thepdf = new RooAddPdf(Form("tot_%s",suffix), Form("tot_%s",suffix), RooArgList(*ws->pdf(Form("sig_%s",suffix)), *ws->pdf(Form("bkg_%s",suffix))));
  ws->import(*thepdf);
