...
bool generateBinned = false;             // for binned generation
   ...
   bool enableDetailedOutput = true;    // for detailed output
...
void StandardHypoTestDemo(const char* infile = "",
                          ...
                          int calcType = 2, // 0 freq 1 hybrid, 2 asymptotic
...
*/
   ...
   if (rndseed >= 0) RooRandom::randomGenerator()->SetSeed(rndseed);
  ...
  if (!bModel ) {
   ...
   RatioOfProfiledLikelihoodsTestStat *
      ropl = new RatioOfProfiledLikelihoodsTestStat(*bModel->GetPdf(), *sbModel->GetPdf(), sbModel->GetParametersOfInterest());
   ...
   if (htr != NULL && writeResult) {
      ...
      if (rndseed>=0) resultFileName += Form("rnd%i_",rndseed);
