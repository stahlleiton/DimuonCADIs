...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   ljpsiresol(TTree *tree=0);
   virtual ~ljpsiresol();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(const char* fname, bool isdata=false, bool ispbpb=false);
...
#ifdef ljpsiresol_cxx
ljpsiresol::ljpsiresol(TTree *tree) : fChain(0) 
{
...
ljpsiresol::~ljpsiresol()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t ljpsiresol::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ljpsiresol::LoadTree(Long64_t entry)
{
...
void ljpsiresol::Init(TTree *tree)
{
...
void ljpsiresol::Show(Long64_t entry)
{
...
Int_t ljpsiresol::Cut(Long64_t entry)
{
...
Bool_t ljpsiresol::isTriggerMatch (Int_t iRecoQQ, Int_t TriggerBit) 
{
   Bool_t cond = true;
...
Bool_t ljpsiresol::isGlobalMuonInAccept2015 (TLorentzVector* Muon) 
{
...
Bool_t ljpsiresol::areMuonsInAcceptance2015 (Int_t iRecoQQ)
{
...
Bool_t ljpsiresol::passQualityCuts2015 (Int_t iRecoQQ) 
{
   Bool_t cond = true;
